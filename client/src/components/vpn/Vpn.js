...
export default function Vpn() {
  ...
  function handleClick(tunnelName, index) {
    // Get the phase2 name
    const phase2Name = tunnels.filter(
  ...
  return [
    <Text fontSize={15} color="white">👉 Instructions: Click the VPN tunnel to gather debugs, once complete click on Run FortiGPT 👈</Text>,

    <Box  my="4">
      
      {tunnels.map((tunnel, index) => (
        <Button  onClick={() => handleClick(tunnel.name, index)} 
        ...
        <Text ml={2} color="white" fontWeight="bold">✨ DEBUG OUTPUT ✨</Text>,
        ...
        <>
        {debugOutput && <Chatgpt debugOutput={debugOutput} chatGptPrompt={vpnDownChatGpt} />}
        </>
