...
export default function systemRoutes(config) {
  ...
  // Route to run the fortiguard script
  router.post("/fortiguard_script", (req, res) => {
    // Execute the script
    const command = `python ./scripts/system/fortiguard.py --device-host ${config.host} --ssh-username ${config.username} --ssh-password ${config.password} --ssh-port ${config.sshPort}`;
